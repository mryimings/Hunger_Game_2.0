...
class Forest:
    def __init__(self, row=25, col=20, tree=25, trap=25, mushroom=25, carnivore=25, disaster_p=0.001):
        ...
        self.observation = np.ones(self.cell_num)  # for dqn as input
    ...
    def get_reward(self, position):
        ...
        if position in self.curr_carnivores:
            ret_reward += -30 * self.curr_carnivores[position]
    ...
    def get_observation(self, position):
        curr_obs = np.zeros(len(self.observation))
        for i in range(len(curr_obs)):
            curr_obs[i] = self.observation[i]
        curr_obs[position] += 7
